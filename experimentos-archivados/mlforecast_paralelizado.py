...
# Configurar logging para suprimir salidas innecesarias
warnings.filterwarnings('ignore')
logging.getLogger('optuna').setLevel(logging.ERROR)
logging.getLogger('lightgbm').setLevel(logging.ERROR)

def optimize_product(pid, df_prod_data, FECHA_CORTE, horizonte_prediccion):
    """
    Función para optimizar un producto específico
    """
    try:
        # Preparar datos del producto
        df_prod = df_prod_data.copy()
        ...
        def objective(trial):
            params = {
                ...
                'random_state': 42,
                'verbose': -1  # Suprimir salidas de LightGBM
            }
            ...
            for train_idx, val_idx in tscv.split(df_entrenamiento_prod):
                ...
                try:
                    fcst = MLForecast(
                        models=LGBMRegressor(**params, device='cpu'),
                        freq='MS',
        ...
        # Optimización bayesiana con logging suprimido
        study = optuna.create_study(direction='minimize')
        study.optimize(objective, n_trials=20, show_progress_bar=False)
        best_params = study.best_params
        best_params['random_state'] = 42
        best_params['verbose'] = -1  # Suprimir salidas de LightGBM

        # Entrenar modelo final y generar predicciones
        fcst_prod = MLForecast(
            models=LGBMRegressor(**best_params, device='cpu'),
            freq='MS',
            lags=list(range(1, 25)),
            date_features=['month', 'year'],
        )
        ...
        result = {
            ...
            'predictions': pred_prod_201912 if not pred_prod_201912.empty else pd.DataFrame()
        }
...
max_workers = min(multiprocessing.cpu_count(), len(product_ids))
print(f"Usando {max_workers} workers para paralelizar la optimización")

with ProcessPoolExecutor(max_workers=max_workers) as executor:
    # Enviar tareas al pool
    futures = {}
    for pid in product_ids:
        df_prod_data = df[df['product_id'] == pid].copy()
        if not df_prod_data.empty:
            future = executor.submit(optimize_product, pid, df_prod_data, FECHA_CORTE, horizonte_prediccion)
            futures[future] = pid
    ...
    for future in as_completed(futures):
        pid = futures[future]
        contador += 1
        print(f"Completado product_id: {pid} ({contador}/{cantidad_total})")
        ...
        try:
            ...
            if not result['predictions'].empty:
                pred_prod_201912 = result['predictions'].copy()
            ...
            print(f"Error obteniendo resultado para product_id {pid}: {e}")
